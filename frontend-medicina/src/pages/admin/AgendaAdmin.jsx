...
// fetchAbonosMes removida - ahora se usa useAbonos hook con paginación automática

  // disparadores
  useEffect(() => {
    // Los abonos ahora se cargan automáticamente con useAbonos hook
    if (isAbonosTab) {
      return;
    }
  ...
  // ── Abonos (listado mensual) - ahora usando hook con paginación automática
  const [y, m] = selectedDate.split("-").map(Number);
  const abonosFilters = useMemo(() => {
    const filters = { anio: y, mes: m };
    if (sedeId) filters.sede_id = Number(sedeId);
    if (prestadorId) filters.prestador_id = Number(prestadorId);
    return filters;
  }, [y, m, sedeId, prestadorId]);
  
  const { abonos: abonosMes, loading: loadingAbonos, error: abonosError, refetch: refetchAbonos } = useAbonos(api, abonosFilters);
