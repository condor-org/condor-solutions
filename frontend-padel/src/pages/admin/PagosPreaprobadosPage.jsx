...
const PagosPreaprobadosPage = () => {
  ...
  useEffect(() => {
    ...
    api.get("pagos/comprobantes/?valido__isnull=True")
  ...
  const handleAccion = async (id, accion) => {
    ...
    try {
      ...
      toast.success(
        accion === "aprobar" ? "Pago confirmado" : "Pago rechazado"
      );
    } catch {
    ...
    <>
      <PageWrapper>
        ...
        <Box flex="1" p={[2, 4, 8]} bg={bg} color={card.color}>
          <Heading size="md" mb={4}>Pagos Preaprobados</Heading>
          {loading ? (
            <Flex align="center" justify="center" h="100px">
              <Spinner color="blue.300" />
            </Flex>
          ) : pagos.length === 0 ? (
            <Text color={mutedText} textAlign="center">No hay pagos para aprobar.</Text>
          ) : (
              ...
              {pagos.map((p) => (
                ...
                >
                  <Flex justify="space-between" align="flex-start" wrap="wrap" gap={3}>
                    <Box minW={["100%", "260px"]}>
                      <Text fontWeight="bold" mb={1}>Comprobante #{p.id}</Text>
                      <Text fontSize="sm" mb={1}>Turno: {p.turno_id}</Text>
