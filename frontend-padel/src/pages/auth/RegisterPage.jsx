...
const RegisterPage = () => {
  ...
  const handleVerifyCode = async (e) => {
    ...
    if (!verificationData.codigo) {
      console.log("[REGISTER] ❌ Código vacío");
      setError("Por favor ingresa el código de verificación");
      return;
    }

    try {
      console.log("[REGISTER] 🔄 Estableciendo isLoading = true");
      console.log("[REGISTER] 🔄 Estado antes de setIsLoading:", { isLoading, error, success });
      setIsLoading(true);
      setError("");
      console.log("[REGISTER] ✅ Estado actualizado - isLoading:", true);
      console.log("[REGISTER] ✅ Error limpiado");
      
      console.log("[REGISTER] 📡 Llamando a verifyCode...");
      console.log("[REGISTER] 📡 Datos enviados a verifyCode:", {
        email: formData.email,
        codigo: verificationData.codigo,
        intent: "registro",
        password: "***"
      });
      console.log("[REGISTER] 📡 verifyCode es una función:", typeof verifyCode);
      console.log("[REGISTER] 📡 verifyCode disponible:", !!verifyCode);
      
      console.log("[REGISTER] ⏳ INICIANDO LLAMADA A verifyCode...");
      ...
      console.log("[REGISTER] ⏳ LLAMADA A verifyCode COMPLETADA");
      
      console.log("[REGISTER] ✅ verifyCode completado exitosamente");
      console.log("[REGISTER] 📦 Resultado de verifyCode:", result);
      setSuccess("¡Registro exitoso! Redirigiendo...");
      console.log("[REGISTER] ✅ Mensaje de éxito establecido");
      
      // Hacer login automático para obtener el rol completo
      try {
        console.log("[REGISTER] 🔐 Iniciando login automático...");
        await login(formData.email, formData.password);
        console.log("[REGISTER] ✅ Login automático exitoso después del registro");
      } catch (loginErr) {
        console.error("[REGISTER] ❌ Error en login automático:", loginErr);
        console.error("[REGISTER] ❌ Continuando de todas formas, el usuario ya está autenticado");
        // Continuar de todas formas, el usuario ya está autenticado
      }
      
      console.log("[REGISTER] ⏰ Programando redirección en 2000ms...");
      setTimeout(() => {
        console.log("[REGISTER] 🚀 Redirigiendo a /jugador (dashboard del usuario)");
        navigate("/jugador");
      }, 2000);
      console.log("[REGISTER] ⏰ Redirección programada");
    } catch (err) {
      console.error("[REGISTER] ❌ ERROR en verifyCode:", err);
      ...
      console.log("[REGISTER] ❌ Error establecido en estado");
    } finally {
      console.log("[REGISTER] 🔄 FINALLY - Estableciendo isLoading = false");
      console.log("[REGISTER] 🔄 Estado antes del finally:", { isLoading, error, success });
      setIsLoading(false);
      console.log("[REGISTER] ✅ Estado final - isLoading:", false);
      console.log("[REGISTER] ===== FIN VERIFICACIÓN =====");
    }
  };
